...
type LibInterface = {
  // 生成目标组件库代码
  generateCode: (data: FormConf, type: SaveConfig["type"], metaInfo: any) => string;
  // 生成sanbbox预览文件
 ...
 * @returns
 */
export function generateCode(data: FormConf, type: SaveConfig["type"], lib: LibType, metaInfo: any): string {
  if (map[lib]) {
