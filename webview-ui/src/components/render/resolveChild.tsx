...
interface pluginInfoType {
  ...
  getOption?: resolveComponentOption;
}
export default class renderChildClass {
  ...
  resolve(): resultInfo {
    ...
    const pluginInfo: pluginInfoType =
      renderChildClass.plugins.get(this.conf.type) || defaultPluginInfo;
    let options = {};
...
class menuPlugin implements pluginInfoType {
  resolveMenu(children: OptionItem[]): Array<any> | string {
    return children.map((item: OptionItem) => {
      if (item.children) {
        return h(
          resolveComponent("el-sub-menu") as ComponentOptions,
          { index: item.value },
          {
...
class tabsPlugin implements pluginInfoType {
  ...
  getChild(conf: ComponentItemJson, opts?: ItemOpts): Array<any> | string {
    return (conf.__slot__?.options || []).map((item: OptionItem, index: number) => {
      return h(resolveComponent("el-tab-pane"), { label: item.label, name: item.value }, () =>
        h(
          "div",
          { class: "drawing-item drawing-row-item" },
          this.draggableRow({
            currentItem: item,
